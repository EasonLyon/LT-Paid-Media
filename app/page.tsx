    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-slate-50 to-sky-50 text-slate-900">
      <div className="mx-auto flex max-w-6xl flex-col gap-12 px-6 py-16 md:px-10 lg:px-12">
        <header className="space-y-4">
          <p className="text-sm uppercase tracking-[0.3em] text-sky-600">Navigation Hub</p>
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <h1 className="text-4xl font-semibold leading-tight text-slate-900 md:text-5xl">Explore every corner</h1>
            <span className="rounded-full border border-slate-200 bg-white px-4 py-2 text-xs text-slate-500 shadow-sm">
        ...
        >
          {subpages.length === 0 ? (
            <div className="col-span-full rounded-3xl border border-slate-200 bg-white p-10 text-center text-slate-500 shadow-xl">
            ...
            subpages.map((page, index) => (
              <Link
                key={page.href}
                href={page.href}
                className={`group relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-7 shadow-xl transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_80px_-30px_rgba(15,23,42,0.35)] ${index % 5 === 0 ? "md:col-span-2" : ""}`}
              >
                ...
                <div className="relative flex items-start justify-between">
                  <div className="flex flex-col gap-2">
                    <span className="text-sm uppercase tracking-[0.2em] text-slate-400">{page.href}</span>
                    <h2 className="text-2xl font-semibold text-slate-900">{page.label}</h2>
                  ...
                  <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-sky-50 text-lg text-sky-700 transition duration-300 group-hover:rotate-6 group-hover:bg-sky-100">
                    ↗
                  </span>
                </div>
              </Link>
