...
export async function GET(req: Request) {
  ...
  try {
    const progress = await readProjectProgress(projectId, "step4-progress.json");
    const hasResultFile = await projectFileExists(projectId, "06-site-keywords-from-top-domains.json");
    if (progress) {
